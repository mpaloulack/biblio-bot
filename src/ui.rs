...
/// Entries a select menu can hold, which also caps the moderator listing.
...
/// The moderator view: everything running in this server, whoever started it.
pub fn admin_watchlist_embed(
    watches: &[Watch],
    now: i64,
    max_age: i64,
    lang: Lang,
    ...
    let embed = serenity::CreateEmbed::new()
        .title(lang.watchlist_admin_title())
        .colour(BLUE);
    if watches.is_empty() {
        return embed.description(lang.watchlist_admin_empty()).colour(GREY);
    }
    ...
    let hidden = watches.len().saturating_sub(MENU_MAX);
    let footer = if hidden > 0 {
        lang.watchlist_more(hidden)
    } else {
        String::new()
    };
    embed.description(format!("{listed}{footer}"))
}

pub fn admin_watchlist_options(
    watches: &[Watch],
    lang: Lang,
    ...
    watches
        ...
        .map(|w| {
            let description = format!(
                "{} · {}",
                lang.watchlist_owner(w.user_id),
                lang.watchlist_entry(w.checks, 0)
            );
            serenity::CreateSelectMenuOption::new(truncate(&w.query, LABEL_MAX), w.id.to_string())
...
mod tests {
    ...
    #[test]
    fn the_moderator_view_names_the_owner_of_each_watch() {
        let mut other = watch("hyperion", 0, 1);
        other.user_id = 99;
        let value = json(admin_watchlist_embed(
            &[watch("dune", 0, 3), other],
            0,
            30 * SECONDS_PER_DAY,
            Lang::En,
        ));
        ...
        assert_eq!(value["title"], "All standing searches");
        assert!(description.contains("<@42>"), "got: {description}");
        assert!(description.contains("<@99>"));
    }

    #[test]
    fn the_moderator_view_says_when_nothing_is_running() {
        let value = json(admin_watchlist_embed(&[], 0, SECONDS_PER_DAY, Lang::Fr));

        assert_eq!(value["color"], GREY);
        assert!(value["description"].as_str().unwrap().contains("Personne"));
    }

    #[test]
    fn the_moderator_view_announces_what_a_menu_cannot_hold() {
        let watches: Vec<Watch> = (0..30)
            .map(|i| {
                let mut w = watch(&format!("book {i}"), 0, 0);
                w.id = i + 1;
                w
            })
            .collect();
        ...
        assert!(description.contains("and 5 more"), "got: {description}");
        assert!(!description.contains("book 25"));
        assert_eq!(admin_watchlist_options(&watches, Lang::En).len(), MENU_MAX);
    }

    #[test]
    fn moderator_options_identify_the_owner_since_mentions_do_not_render_there() {
        let value = serde_json::to_value(admin_watchlist_options(&[watch("dune", 0, 3)], Lang::En))
            .unwrap();
        let description = value[0]["description"].as_str().unwrap();

        assert_eq!(value[0]["value"], "1");
        assert!(description.contains("owner 42"), "got: {description}");
    }

    #[test]
    fn moderator_options_respect_the_discord_length_limit() {
        let value = serde_json::to_value(admin_watchlist_options(
            &[watch(&"z".repeat(250), 0, 0)],
            Lang::En,
        ))
        .unwrap();
